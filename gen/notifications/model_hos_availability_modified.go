/*
Notifications Service - REST API

Notifications Service REST API.

API version: 0.1.0
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package notificationsapi

import (
	"encoding/json"
	"fmt"
	"time"
)

// checks if the HosAvailabilityModified type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &HosAvailabilityModified{}

// HosAvailabilityModified Hos availability modified event.
type HosAvailabilityModified struct {
	// Version of the schema used for this event
	Version   string  `json:"version" validate:"regexp=^\\\\d+\\\\.\\\\d+$"`
	EventName *string `json:"event_name,omitempty"`
	// The event payload, with one or more records of the specified type
	Data []BaseHosAvailability `json:"data"`
	// Unique identifier for the webhook subscription that triggered this delivery
	WebhookId string         `json:"webhook_id"`
	Timestamp NullableTime   `json:"timestamp,omitempty"`
	Id        NullableString `json:"id,omitempty"`
	// The number of times the event has been attempted to be delivered
	DeliveryAttempt      *int32 `json:"delivery_attempt,omitempty"`
	AdditionalProperties map[string]interface{}
}

type _HosAvailabilityModified HosAvailabilityModified

// NewHosAvailabilityModified instantiates a new HosAvailabilityModified object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewHosAvailabilityModified(version string, data []BaseHosAvailability, webhookId string) *HosAvailabilityModified {
	this := HosAvailabilityModified{}
	this.Version = version
	var eventName string = "hos_availability.modified"
	this.EventName = &eventName
	this.Data = data
	this.WebhookId = webhookId
	var deliveryAttempt int32 = 0
	this.DeliveryAttempt = &deliveryAttempt
	return &this
}

// NewHosAvailabilityModifiedWithDefaults instantiates a new HosAvailabilityModified object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewHosAvailabilityModifiedWithDefaults() *HosAvailabilityModified {
	this := HosAvailabilityModified{}
	var eventName string = "hos_availability.modified"
	this.EventName = &eventName
	var deliveryAttempt int32 = 0
	this.DeliveryAttempt = &deliveryAttempt
	return &this
}

// GetVersion returns the Version field value
func (o *HosAvailabilityModified) GetVersion() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.Version
}

// GetVersionOk returns a tuple with the Version field value
// and a boolean to check if the value has been set.
func (o *HosAvailabilityModified) GetVersionOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Version, true
}

// SetVersion sets field value
func (o *HosAvailabilityModified) SetVersion(v string) {
	o.Version = v
}

// GetEventName returns the EventName field value if set, zero value otherwise.
func (o *HosAvailabilityModified) GetEventName() string {
	if o == nil || IsNil(o.EventName) {
		var ret string
		return ret
	}
	return *o.EventName
}

// GetEventNameOk returns a tuple with the EventName field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *HosAvailabilityModified) GetEventNameOk() (*string, bool) {
	if o == nil || IsNil(o.EventName) {
		return nil, false
	}
	return o.EventName, true
}

// HasEventName returns a boolean if a field has been set.
func (o *HosAvailabilityModified) HasEventName() bool {
	if o != nil && !IsNil(o.EventName) {
		return true
	}

	return false
}

// SetEventName gets a reference to the given string and assigns it to the EventName field.
func (o *HosAvailabilityModified) SetEventName(v string) {
	o.EventName = &v
}

// GetData returns the Data field value
func (o *HosAvailabilityModified) GetData() []BaseHosAvailability {
	if o == nil {
		var ret []BaseHosAvailability
		return ret
	}

	return o.Data
}

// GetDataOk returns a tuple with the Data field value
// and a boolean to check if the value has been set.
func (o *HosAvailabilityModified) GetDataOk() ([]BaseHosAvailability, bool) {
	if o == nil {
		return nil, false
	}
	return o.Data, true
}

// SetData sets field value
func (o *HosAvailabilityModified) SetData(v []BaseHosAvailability) {
	o.Data = v
}

// GetWebhookId returns the WebhookId field value
func (o *HosAvailabilityModified) GetWebhookId() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.WebhookId
}

// GetWebhookIdOk returns a tuple with the WebhookId field value
// and a boolean to check if the value has been set.
func (o *HosAvailabilityModified) GetWebhookIdOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.WebhookId, true
}

// SetWebhookId sets field value
func (o *HosAvailabilityModified) SetWebhookId(v string) {
	o.WebhookId = v
}

// GetTimestamp returns the Timestamp field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *HosAvailabilityModified) GetTimestamp() time.Time {
	if o == nil || IsNil(o.Timestamp.Get()) {
		var ret time.Time
		return ret
	}
	return *o.Timestamp.Get()
}

// GetTimestampOk returns a tuple with the Timestamp field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *HosAvailabilityModified) GetTimestampOk() (*time.Time, bool) {
	if o == nil {
		return nil, false
	}
	return o.Timestamp.Get(), o.Timestamp.IsSet()
}

// HasTimestamp returns a boolean if a field has been set.
func (o *HosAvailabilityModified) HasTimestamp() bool {
	if o != nil && o.Timestamp.IsSet() {
		return true
	}

	return false
}

// SetTimestamp gets a reference to the given NullableTime and assigns it to the Timestamp field.
func (o *HosAvailabilityModified) SetTimestamp(v time.Time) {
	o.Timestamp.Set(&v)
}

// SetTimestampNil sets the value for Timestamp to be an explicit nil
func (o *HosAvailabilityModified) SetTimestampNil() {
	o.Timestamp.Set(nil)
}

// UnsetTimestamp ensures that no value is present for Timestamp, not even an explicit nil
func (o *HosAvailabilityModified) UnsetTimestamp() {
	o.Timestamp.Unset()
}

// GetId returns the Id field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *HosAvailabilityModified) GetId() string {
	if o == nil || IsNil(o.Id.Get()) {
		var ret string
		return ret
	}
	return *o.Id.Get()
}

// GetIdOk returns a tuple with the Id field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *HosAvailabilityModified) GetIdOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return o.Id.Get(), o.Id.IsSet()
}

// HasId returns a boolean if a field has been set.
func (o *HosAvailabilityModified) HasId() bool {
	if o != nil && o.Id.IsSet() {
		return true
	}

	return false
}

// SetId gets a reference to the given NullableString and assigns it to the Id field.
func (o *HosAvailabilityModified) SetId(v string) {
	o.Id.Set(&v)
}

// SetIdNil sets the value for Id to be an explicit nil
func (o *HosAvailabilityModified) SetIdNil() {
	o.Id.Set(nil)
}

// UnsetId ensures that no value is present for Id, not even an explicit nil
func (o *HosAvailabilityModified) UnsetId() {
	o.Id.Unset()
}

// GetDeliveryAttempt returns the DeliveryAttempt field value if set, zero value otherwise.
func (o *HosAvailabilityModified) GetDeliveryAttempt() int32 {
	if o == nil || IsNil(o.DeliveryAttempt) {
		var ret int32
		return ret
	}
	return *o.DeliveryAttempt
}

// GetDeliveryAttemptOk returns a tuple with the DeliveryAttempt field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *HosAvailabilityModified) GetDeliveryAttemptOk() (*int32, bool) {
	if o == nil || IsNil(o.DeliveryAttempt) {
		return nil, false
	}
	return o.DeliveryAttempt, true
}

// HasDeliveryAttempt returns a boolean if a field has been set.
func (o *HosAvailabilityModified) HasDeliveryAttempt() bool {
	if o != nil && !IsNil(o.DeliveryAttempt) {
		return true
	}

	return false
}

// SetDeliveryAttempt gets a reference to the given int32 and assigns it to the DeliveryAttempt field.
func (o *HosAvailabilityModified) SetDeliveryAttempt(v int32) {
	o.DeliveryAttempt = &v
}

func (o HosAvailabilityModified) MarshalJSON() ([]byte, error) {
	toSerialize, err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o HosAvailabilityModified) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	toSerialize["version"] = o.Version
	if !IsNil(o.EventName) {
		toSerialize["event_name"] = o.EventName
	}
	toSerialize["data"] = o.Data
	toSerialize["webhook_id"] = o.WebhookId
	if o.Timestamp.IsSet() {
		toSerialize["timestamp"] = o.Timestamp.Get()
	}
	if o.Id.IsSet() {
		toSerialize["id"] = o.Id.Get()
	}
	if !IsNil(o.DeliveryAttempt) {
		toSerialize["delivery_attempt"] = o.DeliveryAttempt
	}

	for key, value := range o.AdditionalProperties {
		toSerialize[key] = value
	}

	return toSerialize, nil
}

func (o *HosAvailabilityModified) UnmarshalJSON(data []byte) (err error) {
	// This validates that all required properties are included in the JSON object
	// by unmarshalling the object into a generic map with string keys and checking
	// that every required field exists as a key in the generic map.
	requiredProperties := []string{
		"version",
		"data",
		"webhook_id",
	}

	allProperties := make(map[string]interface{})

	err = json.Unmarshal(data, &allProperties)

	if err != nil {
		return err
	}

	for _, requiredProperty := range requiredProperties {
		if _, exists := allProperties[requiredProperty]; !exists {
			return fmt.Errorf("no value given for required property %v", requiredProperty)
		}
	}

	varHosAvailabilityModified := _HosAvailabilityModified{}

	err = json.Unmarshal(data, &varHosAvailabilityModified)

	if err != nil {
		return err
	}

	*o = HosAvailabilityModified(varHosAvailabilityModified)

	additionalProperties := make(map[string]interface{})

	if err = json.Unmarshal(data, &additionalProperties); err == nil {
		delete(additionalProperties, "version")
		delete(additionalProperties, "event_name")
		delete(additionalProperties, "data")
		delete(additionalProperties, "webhook_id")
		delete(additionalProperties, "timestamp")
		delete(additionalProperties, "id")
		delete(additionalProperties, "delivery_attempt")
		o.AdditionalProperties = additionalProperties
	}

	return err
}

type NullableHosAvailabilityModified struct {
	value *HosAvailabilityModified
	isSet bool
}

func (v NullableHosAvailabilityModified) Get() *HosAvailabilityModified {
	return v.value
}

func (v *NullableHosAvailabilityModified) Set(val *HosAvailabilityModified) {
	v.value = val
	v.isSet = true
}

func (v NullableHosAvailabilityModified) IsSet() bool {
	return v.isSet
}

func (v *NullableHosAvailabilityModified) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableHosAvailabilityModified(val *HosAvailabilityModified) *NullableHosAvailabilityModified {
	return &NullableHosAvailabilityModified{value: val, isSet: true}
}

func (v NullableHosAvailabilityModified) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableHosAvailabilityModified) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}
