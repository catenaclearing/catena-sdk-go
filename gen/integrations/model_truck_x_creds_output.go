/*
Telematics Integrations Service - REST API

Integrations Service REST API.

API version: 0.1.0
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package integrationsapi

import (
	"encoding/json"
	"fmt"
)

// checks if the TruckXCredsOutput type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &TruckXCredsOutput{}

// TruckXCredsOutput TruckX Short-lived Credentials model
type TruckXCredsOutput struct {
	DotNumber            string      `json:"dot_number"`
	Token                interface{} `json:"token"`
	AdditionalProperties map[string]interface{}
}

type _TruckXCredsOutput TruckXCredsOutput

// NewTruckXCredsOutput instantiates a new TruckXCredsOutput object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewTruckXCredsOutput(dotNumber string, token interface{}) *TruckXCredsOutput {
	this := TruckXCredsOutput{}
	this.DotNumber = dotNumber
	this.Token = token
	return &this
}

// NewTruckXCredsOutputWithDefaults instantiates a new TruckXCredsOutput object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewTruckXCredsOutputWithDefaults() *TruckXCredsOutput {
	this := TruckXCredsOutput{}
	return &this
}

// GetDotNumber returns the DotNumber field value
func (o *TruckXCredsOutput) GetDotNumber() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.DotNumber
}

// GetDotNumberOk returns a tuple with the DotNumber field value
// and a boolean to check if the value has been set.
func (o *TruckXCredsOutput) GetDotNumberOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.DotNumber, true
}

// SetDotNumber sets field value
func (o *TruckXCredsOutput) SetDotNumber(v string) {
	o.DotNumber = v
}

// GetToken returns the Token field value
// If the value is explicit nil, the zero value for interface{} will be returned
func (o *TruckXCredsOutput) GetToken() interface{} {
	if o == nil {
		var ret interface{}
		return ret
	}

	return o.Token
}

// GetTokenOk returns a tuple with the Token field value
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *TruckXCredsOutput) GetTokenOk() (*interface{}, bool) {
	if o == nil || IsNil(o.Token) {
		return nil, false
	}
	return &o.Token, true
}

// SetToken sets field value
func (o *TruckXCredsOutput) SetToken(v interface{}) {
	o.Token = v
}

func (o TruckXCredsOutput) MarshalJSON() ([]byte, error) {
	toSerialize, err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o TruckXCredsOutput) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	toSerialize["dot_number"] = o.DotNumber
	if o.Token != nil {
		toSerialize["token"] = o.Token
	}

	for key, value := range o.AdditionalProperties {
		toSerialize[key] = value
	}

	return toSerialize, nil
}

func (o *TruckXCredsOutput) UnmarshalJSON(data []byte) (err error) {
	// This validates that all required properties are included in the JSON object
	// by unmarshalling the object into a generic map with string keys and checking
	// that every required field exists as a key in the generic map.
	requiredProperties := []string{
		"dot_number",
		"token",
	}

	allProperties := make(map[string]interface{})

	err = json.Unmarshal(data, &allProperties)

	if err != nil {
		return err
	}

	for _, requiredProperty := range requiredProperties {
		if _, exists := allProperties[requiredProperty]; !exists {
			return fmt.Errorf("no value given for required property %v", requiredProperty)
		}
	}

	varTruckXCredsOutput := _TruckXCredsOutput{}

	err = json.Unmarshal(data, &varTruckXCredsOutput)

	if err != nil {
		return err
	}

	*o = TruckXCredsOutput(varTruckXCredsOutput)

	additionalProperties := make(map[string]interface{})

	if err = json.Unmarshal(data, &additionalProperties); err == nil {
		delete(additionalProperties, "dot_number")
		delete(additionalProperties, "token")
		o.AdditionalProperties = additionalProperties
	}

	return err
}

type NullableTruckXCredsOutput struct {
	value *TruckXCredsOutput
	isSet bool
}

func (v NullableTruckXCredsOutput) Get() *TruckXCredsOutput {
	return v.value
}

func (v *NullableTruckXCredsOutput) Set(val *TruckXCredsOutput) {
	v.value = val
	v.isSet = true
}

func (v NullableTruckXCredsOutput) IsSet() bool {
	return v.isSet
}

func (v *NullableTruckXCredsOutput) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableTruckXCredsOutput(val *TruckXCredsOutput) *NullableTruckXCredsOutput {
	return &NullableTruckXCredsOutput{value: val, isSet: true}
}

func (v NullableTruckXCredsOutput) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableTruckXCredsOutput) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}
